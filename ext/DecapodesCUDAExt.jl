...
function default_dec_cu_matrix_generate(sd::HasDeltaSet, my_symbol::Symbol, hodge::DiscreteHodge)
  op = @match my_symbol begin
    ...
    # Wedge Products
    :∧₀₁ => dec_cu_pair_wedge_product(Val(0), Val(1), sd)
    :∧₁₀ => dec_cu_pair_wedge_product(Val(1), Val(0), sd)
    :∧₀₂ => dec_cu_pair_wedge_product(Val(0), Val(2), sd)
    :∧₂₀ => dec_cu_pair_wedge_product(Val(2), Val(0), sd)
    :∧₁₁ => dec_cu_pair_wedge_product(Val(1), Val(1), sd)
...
function dec_cu_mat_hodge(::Val{1}, sd::HasDeltaSet, hodge::DiagonalHodge)
  hodge = dec_hodge_star(1, sd, hodge, Val(:CUDA))
  return (hodge, x -> hodge * x)
end

function dec_cu_mat_hodge(::Val{1}, sd::HasDeltaSet, hodge::GeometricHodge)
...
"""dec_cu_pair_inv_hodge(::Val{1}, sd::EmbeddedDeltaDualComplex2D{Bool, float_type, _p} where _p,  ::GeometricHodge ) where float_type
...
"""
function dec_cu_pair_inv_hodge(::Val{1}, sd::EmbeddedDeltaDualComplex2D{Bool, float_type, _p} where _p,  ::GeometricHodge ) where float_type
...
function dec_cu_pair_inv_hodge(::Val{1}, sd::HasDeltaSet, ::DiagonalHodge)
...
function dec_cu_pair_wedge_product(::Val{k}, ::Val{0}, sd::HasDeltaSet) where {k}
  val_pack = cache_wedge(Val(0), Val(k), sd, Val(:CUDA))
  ((y, α, g) -> dec_c_wedge_product!(Val(0), Val(k), y, g, α, val_pack[1], val_pack[2]),
    (α, g) -> dec_c_wedge_product(Val(0), Val(k), g, α, val_pack))
end

function dec_cu_pair_wedge_product(::Val{0}, ::Val{k}, sd::HasDeltaSet) where {k}
  val_pack = cache_wedge(Val(0), Val(k), sd, Val(:CUDA))
  ((y, f, β) -> dec_c_wedge_product!(Val(0), Val(k), y, f, β, val_pack[1], val_pack[2]),
    (f, β) -> dec_c_wedge_product(Val(0), Val(k), f, β, val_pack))
end

function dec_cu_pair_wedge_product(::Val{1}, ::Val{1}, sd::HasDeltaSet2D)
  val_pack = cache_wedge(Val(1), Val(1), sd, Val(:CUDA))
