...
function simulate(mesh, operators, hodge = GeometricHodge())
  ...
  begin
    var"GenSim-M_GenSim-ConMat_1" = CuSparseMatrixCSC(var"GenSim-M_⋆₀⁻¹" * var"GenSim-M_dual_d₁" * var"GenSim-M_⋆₁")
    var"GenSim-M_d₀" = CuSparseMatrixCSC(var"GenSim-M_d₀")
    val_pack_01 = dec_p_wedge_product(Tuple{0,1}, mesh)
    primal_vertices_01 = CuArray(val_pack_01[1])
    var"GenSim-M_∧₁₀" = (w, a, f) -> dec_cu_c_wedge_product!(Tuple{0, 1}, w, f, a, primal_vertices_01)
  end
  begin
    var"dynamics_•4" = CuVector{Float64}(undef, nparts(mesh, :E))
    var"dynamics_•8" = CuVector{Float64}(undef, nparts(mesh, :E))
    var"dynamics_•3" = CuVector{Float64}(undef, nparts(mesh, :E))
    var"dynamics_•2" = CuVector{Float64}(undef, nparts(mesh, :E))
    var"dynamics_•1" = CuVector{Float64}(undef, nparts(mesh, :E))
    dynamics_ḣ = CuVector{Float64}(undef, nparts(mesh, :V))
  end
  f(du, u, p, t) = begin
    begin
    ...
    mul!(var"dynamics_•4", var"GenSim-M_d₀", dynamics_h)
    mul!(var"dynamics_•8", var"GenSim-M_d₀", dynamics_h)
    var"dynamics_•7" = ♯(var"dynamics_•8")
    var"dynamics_•6" = mag(var"dynamics_•7")
    var"dynamics_•9" = n .- var"1"
    var"dynamics_•5" = var"dynamics_•6" .^ var"dynamics_•9"
    var"stress_•3" = stress_ρ .* stress_g
    var"stress_•2" = var"stress_•3" .^ n
    dynamics_sum_1 = (.+)(n, var"2")
    stress_sum_1 = (.+)(n, var"2")
    var"dynamics_•10" = dynamics_h .^ dynamics_sum_1
    var"stress_•1" = var"2" / stress_sum_1
    stress_mult_1 = var"stress_•1" .* stress_A
    Γ = stress_mult_1 .* var"stress_•2"
    var"dynamics_•3" .= Γ .* var"dynamics_•4"
    var"GenSim-M_∧₁₀"(var"dynamics_•2", var"dynamics_•3", var"dynamics_•5")
    var"GenSim-M_∧₁₀"(var"dynamics_•1", var"dynamics_•2", var"dynamics_•10")
    mul!(dynamics_ḣ, var"GenSim-M_GenSim-ConMat_1", var"dynamics_•1")
    getproperty(du, :dynamics_h) .= dynamics_ḣ
  end
end
        
fₘ = simulate(s, generate)
