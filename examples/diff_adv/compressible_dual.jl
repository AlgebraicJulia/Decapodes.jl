...
dd0_hdg2 = CuSparseMatrixCSC{Float64}(special_dual_d0 * hdg_2)

codif_1 = CuSparseMatrixCSC{Float64}(hdg_2) * CuSparseMatrixCSC{Float64}(d1) * invhdg1
...
timestep = length(Us)
u_end = cpu_wdg_10dd(Us[timestep], 1 ./ rhos[timestep])
...
function save_dynamics(save_file_name)
  ...
  w = @lift(inv_hdg_0 * dual_d1 * cpu_wdg_10dd(Us[$time], 1 ./ rhos[$time]))
  ρ = @lift(interp * rhos[$time])
