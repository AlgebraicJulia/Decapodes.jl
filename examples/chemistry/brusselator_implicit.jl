...
@time soln_im = solve(prob, FBDF(linsolve = KLUFactorization()), progress=true, progress_steps=1, saveat=0.1);
@time soln_ex = solve(prob, Tsit5(), progress=true, progress_steps=1, saveat=0.1);

function save_dynamics(soln, save_file_name)
  time = Observable(0.0)
  u = @lift(soln($time).U)
  f = Figure()
...
save_dynamics(soln_im, "brusselator_implicit.gif")
save_dynamics(soln_ex, "brusselator_explicit.gif")
