...
used by [`gensim`](@ref), [`gen_int`](@ref), and [`gen_split`](@ref).
...
"""
    _gen_runtime_defs(c; include_nanmath::Bool, include_multigrid::Bool)
...
"""
    _gen_split_branch(name::Symbol, c)
...
function validate_split_tangent_states(implicit_d::SummationDecapode, explicit_d::SummationDecapode)
  ...
  Set(implicit_states) == Set(explicit_states) && return nothing

  implicit_msg = join(sort!(string.(implicit_states)), ", ")
  explicit_msg = join(sort!(string.(explicit_states)), ", ")
...
"""
    gen_split(implicit_d::SummationDecapode, explicit_d::SummationDecapode, input_vars::Vector{Symbol}; dimension::Int=2, stateeltype::DataType = Float64, code_target::AbstractGenerationTarget = CPUTarget(), preallocate::Bool = true, contract::Bool = true, multigrid::Bool = false, cse::Bool = true, nanmath_support::Bool = false)

Generate code for a split ODE with IMEX-compatible in-place function signatures.

The returned expression, when evaluated, produces a function with the signature:
`(mesh, operators, hodge) -> (f_implicit, f_explicit)`.

Each returned inner function has the same in-place signature used by `gensim`,
namely `(__du__, __u__, __p__, __t__) -> nothing`, and is therefore compatible
with `SciMLBase.SplitFunction` / `SplitODEProblem` style APIs expected by IMEX
solvers. Following SciML split ODE conventions, `f_implicit` is returned first
and `f_explicit` second.

Both Decapodes must define the same `∂ₜ` state variables.
...
function gen_split(user_implicit_d::SummationDecapode, user_explicit_d::SummationDecapode, input_vars::Vector{Symbol}; dimension::Int=2, stateeltype::DataType = Float64, code_target::AbstractGenerationTarget = CPUTarget(), preallocate::Bool = true, contract::Bool = true, multigrid::Bool = false, cse::Bool = true, nanmath_support::Bool = false)
  ...
  c_implicit = _compile_decapode(implicit_d, implicit_input_vars, [];
    dimension, stateeltype, code_target, preallocate, contract, cse,
    gen_tars = true, multigrid, nanmath_support)
  c_explicit = _compile_decapode(explicit_d, explicit_input_vars, [];
    dimension, stateeltype, code_target, preallocate, contract, cse,
    gen_tars = true, multigrid, nanmath_support)
...
gen_split(implicit_d::SummationDecapode, explicit_d::SummationDecapode; kwargs...) =
  gen_split(implicit_d, explicit_d, gather_split_inputs(implicit_d, explicit_d); kwargs...)

gen_split(implicit_c::Collage, explicit_c::Collage; kwargs...) =
